...
class TaskService {
  ...
  async createTask(data: CreateTaskRequest): Promise<CrawlTask> {
    const response = await wechatHttp.post<CreateTaskResponse>('/tasks/', data);
  ...
  async getTaskList(params: TaskListParams = {}): Promise<TaskListResponse['data']> {
    const response = await wechatHttp.get<TaskListResponse>('/tasks/', params);
    return response.data.data;
  }
  ...
  async getTaskDetail(taskId: string): Promise<CrawlTask> {
    const response = await wechatHttp.get<TaskDetailResponse>(`/tasks/${taskId}`);
    return response.data.data;
  }
  ...
  async updateTask(taskId: string, data: UpdateTaskRequest): Promise<CrawlTask> {
    const response = await wechatHttp.put<TaskDetailResponse>(`/tasks/${taskId}`, data);
    return response.data.data;
  }
  ...
  async executeTask(taskId: string): Promise<string> {
    const response = await wechatHttp.post<ExecuteTaskResponse>(`/tasks/${taskId}/execute`);
    return response.data.data;
  }
  ...
  async deleteTask(taskId: string): Promise<string> {
    const response = await wechatHttp.delete<DeleteTaskResponse>(`/tasks/${taskId}`);
    return response.data.data;
  }
